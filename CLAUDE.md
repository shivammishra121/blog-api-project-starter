...
- Endpoints: `POST /api/posts?title=&content=`, `GET /api/posts`, `GET /api/posts/{id}`, `DELETE /api/posts/{id}`, `POST /api/posts/validate?content=` (max 5000 chars), `GET /api/posts/total`.
...
- **Crashes:** `GET`/`DELETE /{id}` with a missing ID throw `IndexOutOfBoundsException` → HTTP 500 instead of 404.
